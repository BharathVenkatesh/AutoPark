...
TIM_HandleTypeDef tim_init_vec[5];
TIM_Base_InitTypeDef tim_conf_vec[5];
...
void init_timers() {
	...
	for (int i = 0; i < 5; ++i)
	{
		if (i == RIGHT)
			...
			tim_init_vec[i].Instance = TIM6;
		else if (i == ENCLEFT)
			tim_init_vec[i].Instance = TIM7;
		...
		if (i != ENCLEFT && i != ENCRIGHT)
			tim_init_vec[i].Init.Prescaler = 72;
		else tim_init_vec[i].Init.Prescaler = 72000;
		...
		else tim_conf_vec[i].Prescaler = 72000;
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
	if (GPIO_Pin == echosPins.right)
		...
		encoders_Callback(encodersPins.right, tim_init_vec[ENCRIGHT], GPIOA);
	}
	else if (GPIO_Pin == encodersPins.left)
		encoders_Callback(encodersPins.left, tim_init_vec[ENCLEFT], GPIOA);
}
